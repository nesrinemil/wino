...
#include "../han/SmartDrivingSchool/circuitdashboard.h"
#include "../han/SmartDrivingSchool/circuitdb.h"
#include <QVBoxLayout>
#include <QHBoxLayout>
#include <QGridLayout>
#include <QFrame>
...
void InstructorDashboard::setupUI()
{
    ...
    connect(theme, &ThemeManager::themeChanged, this, &InstructorDashboard::onThemeChanged);
    ...
    connect(backBtn, &QPushButton::clicked, this, &InstructorDashboard::onBackClicked);
    ...
    QString instructorD17 = "N/A";
    {
        int instId = qApp->property("currentUserId").toInt();
        QSqlQuery qInstD17;
        qInstD17.prepare("SELECT d17_id FROM INSTRUCTORS WHERE id = :id");
    ...
    {
        QSqlQuery qid;
        qid.prepare("SELECT driving_school_id FROM INSTRUCTORS WHERE id = :id");
    ...
    {
        QSqlQuery qPayments;
        qPayments.prepare("SELECT COUNT(*) FROM PAYMENT p JOIN STUDENTS s ON p.student_id = s.id WHERE p.status = 'PENDING' AND s.driving_school_id = :school_id");
...
// ══════════════ STATS CARD ══════════════
QWidget* InstructorDashboard::createStatsCard(const QString& title, const QString& value,
...
// ══════════════ SPECIALTIES ══════════════
QWidget* InstructorDashboard::createSpecialtiesSection()
{
    ThemeManager* theme = ThemeManager::instance();
    ...
    int instId = qApp->property("currentUserId").toInt();
    QSqlQuery qSpec;
    qSpec.prepare("SELECT specialization FROM INSTRUCTORS WHERE id = :id");
...
// ══════════════ TAB BAR ══════════════
QWidget* InstructorDashboard::createTabBar()
{
    ...
    connect(scheduleTab, &QPushButton::clicked, this, &InstructorDashboard::onScheduleTabClicked);
    ...
    connect(paymentTab, &QPushButton::clicked, this, &InstructorDashboard::onPaymentTabClicked);
    ...
    connect(examsTab, &QPushButton::clicked, this, &InstructorDashboard::onExamsTabClicked);
    ...
    connect(studentsTab, &QPushButton::clicked, this, &InstructorDashboard::onStudentsTabClicked);
    
    circuitTabBtn = new QPushButton("🚦  Circuit Analysis");
    circuitTabBtn->setStyleSheet(
        "QPushButton { background-color: transparent; color: #0D9488; font-size: 15px; font-weight: 600;"
        "    border: none; border-radius: 10px; padding: 14px 30px; }"
        "QPushButton:hover { color: #111827; background-color: #F0FDFA; }"
    );
    circuitTabBtn->setCursor(Qt::PointingHandCursor);
    connect(circuitTabBtn, &QPushButton::clicked, this, [this]() {
        // Open the friend's Circuit Dashboard
        int instructorId = qApp->property("currentUserId").toInt();
        int schoolId = 5; // fallback
        QSqlQuery qid("SELECT driving_school_id FROM INSTRUCTORS WHERE id = " + QString::number(instructorId));
        if (qid.next()) schoolId = qid.value(0).toInt();
        ...
        if (!CircuitDB::instance()->isOpen()) {
            ...
            if (!CircuitDB::instance()->initialize(p)) {
                QMessageBox::critical(this, "Circuit DB Error", CircuitDB::instance()->lastError());
                return;
            }
        }
    ...
    tabLayout->addWidget(circuitTabBtn);
    tabLayout->addStretch();
    return tabBar;
}

// ══════════════ WEEKLY SCHEDULE (shell + dynamic grid) ══════════════
QWidget* InstructorDashboard::createWeeklySchedule()
{
    ThemeManager* theme = ThemeManager::instance();
    ...
    connect(prevBtn, &QPushButton::clicked, this, &InstructorDashboard::onPrevWeek);
    ...
    connect(nextBtn, &QPushButton::clicked, this, &InstructorDashboard::onNextWeek);
...
void InstructorDashboard::rebuildScheduleGrid()
{
    ThemeManager* theme = ThemeManager::instance();
    bool isDark = theme->currentTheme() == ThemeManager::Dark;
    ...
    QSqlQuery qBooked;
    qBooked.prepare("SELECT TO_CHAR(sb.session_date, 'YYYY-MM-DD'), sb.start_time, s.first_name || ' ' || s.last_name, sb.end_time "
...
void InstructorDashboard::updateSlotSummary()
{
    // This is handled inside rebuildScheduleGrid now
}

// ══════════════ CELL CLICKED — toggle availability ══════════════
void InstructorDashboard::onCellClicked(int dayOffset, int hour)
{
...
void InstructorDashboard::onDayHeaderClicked(int dayOffset)
{
    QDate dayDate = currentWeekStart.addDays(dayOffset);
    showDaySummary(dayDate);
}

void InstructorDashboard::showDaySummary(const QDate& date)
{
    ThemeManager* theme = ThemeManager::instance();
    bool isDark = theme->currentTheme() == ThemeManager::Dark;
...
// ══════════════ PAYMENT SECTION ══════════════
QWidget* InstructorDashboard::createPaymentSection()
{
    ThemeManager* theme = ThemeManager::instance();
    ...
    QSqlQuery q;
    q.prepare("SELECT p.payment_id, s.first_name || ' ' || s.last_name as student_name, "
              "p.d17_code, TO_CHAR(p.facture_date, 'DD/MM/YYYY HH24:MI'), "
              "p.amount "
              "FROM PAYMENT p JOIN STUDENTS s ON p.student_id = s.id "
              "WHERE p.status = 'PENDING' AND s.instructor_id = :instructor_id "
...
QWidget* InstructorDashboard::createPaymentCard(int paymentId, const QString& studentName, const QString& sessionType,
...
// ══════════════ WEEK DISPLAY ══════════════
void InstructorDashboard::updateWeekDisplay()
{
...
// ══════════════ SLOTS ══════════════
void InstructorDashboard::onBackClicked()
{
    this->close();
    if (parentWidget()) parentWidget()->showNormal();
}

void InstructorDashboard::onScheduleTabClicked()
{
    tabContent->setCurrentIndex(0);
    scheduleTab->setStyleSheet(
        "QPushButton { background-color: #14B8A6; color: white; font-size: 15px; font-weight: bold;"
...
void InstructorDashboard::onPaymentTabClicked()
{
...
void InstructorDashboard::onExamsTabClicked()
{
...
void InstructorDashboard::onStudentsTabClicked()
{
...
void InstructorDashboard::onPrevWeek()
{
...
void InstructorDashboard::onNextWeek()
{
    currentWeekStart = currentWeekStart.addDays(7);
    updateWeekDisplay();
    rebuildScheduleGrid();
...
void InstructorDashboard::onVerifyPayment(int paymentId, const QString& studentName)
{
...
void InstructorDashboard::onRejectPayment(int paymentId, const QString& studentName)
{
...
void InstructorDashboard::updateColors()
{
    ThemeManager* theme = ThemeManager::instance();
    bool isDark = theme->currentTheme() == ThemeManager::Dark;
...
void InstructorDashboard::onThemeChanged()
{
    updateColors();
...
void InstructorDashboard::showBookingDetails(const QString& key)
{
    ...
    QSqlQuery q;
    q.prepare("SELECT s.first_name || ' ' || s.last_name, s.phone, s.email, sb.start_time, sb.end_time, sb.amount, sb.status "
              "FROM SESSION_BOOKING sb JOIN STUDENTS s ON sb.student_id = s.id "
...
QWidget* InstructorDashboard::createExamsSection()
{
    ThemeManager* theme = ThemeManager::instance();
    bool isDark = theme->currentTheme() == ThemeManager::Dark;
    ...
    q.prepare(
        ...
        "LEFT JOIN USER_PROGRESS up ON up.user_id = s.id "
...
void InstructorDashboard::onAcceptExamRequest(int requestId)
{
    ThemeManager* theme = ThemeManager::instance();
    bool isDark = theme->currentTheme() == ThemeManager::Dark;
    ...
    {
        QSqlQuery q;
        q.prepare("SELECT s.email, s.first_name || ' ' || s.last_name, er.EXAM_STEP "
...
"</div></body></html>")
        ...
        if (ok) QMessageBox::information(this, "Email Sent", "Exam approved and email sent to " + studentEmail + "!");
        else    QMessageBox::warning(this, "Email Error", "DB updated but email failed:\n" + err);
...
void InstructorDashboard::onRecordExamResult(int requestId)
{
    ...
    QSqlQuery q;
    q.prepare("SELECT er.STUDENT_ID, s.first_name || ' ' || s.last_name, er.EXAM_STEP "
              "FROM EXAM_REQUEST er JOIN STUDENTS s ON er.STUDENT_ID = s.id "
              "WHERE er.REQUEST_ID = :id");
    q.bindValue(":id", requestId);
    if (q.exec() && q.next()) {
    ...
    connect(saveBtn, &QPushButton::clicked, [=]() {
        ...
        // 2. If student passed → advance USER_PROGRESS
...
void InstructorDashboard::onRejectExamRequest(int requestId)
{
    ThemeManager* theme = ThemeManager::instance();
    bool isDark = theme->currentTheme() == ThemeManager::Dark;
...
"</div></body></html>")
        ...
        dlg->accept();
        onExamsTabClicked();
        if (ok) QMessageBox::information(this, "Done", "Request rejected and email sent to " + studentEmail + "!");
        else    QMessageBox::warning(this, "Email Error", "DB updated but email failed:\n" + err);
    });
...
QWidget* InstructorDashboard::createStudentsSection()
{
    ThemeManager* theme = ThemeManager::instance();
    bool isDark = theme->currentTheme() == ThemeManager::Dark;
    ...
    QPushButton *reqBtn = new QPushButton("🔔 Pending Requests");
    reqBtn->setStyleSheet(
        "QPushButton { background-color: #F59E0B; color: white; font-size: 14px; font-weight: bold;"
        "    border: none; border-radius: 8px; padding: 8px 16px; }"
        "QPushButton:hover { background-color: #D97706; }"
    );
    reqBtn->setCursor(Qt::PointingHandCursor);
    connect(reqBtn, &QPushButton::clicked, this, &InstructorDashboard::showPendingRequests);
    headerLayout->addWidget(reqBtn);
    ...
    QSqlQuery q;
    q.prepare("SELECT s.id, s.first_name || ' ' || s.last_name, s.phone, s.email, "
              "       NVL(up.current_step, 1), up.code_status, up.circuit_status, up.parking_status, "
              "       (SELECT COUNT(*) FROM SESSION_BOOKING sb WHERE sb.student_id = s.id AND sb.status IN ('CONFIRMED', 'COMPLETED') AND sb.SESSION_DATE < SYSDATE), "
              "       up.is_qualified, "
              "       (SELECT TO_CHAR(MIN(CONFIRMED_DATE), 'DD/MM/YYYY') FROM EXAM_REQUEST er WHERE er.STUDENT_ID = s.id AND er.STATUS = 'APPROVED' AND er.CONFIRMED_DATE >= TRUNC(SYSDATE)) as next_exam "
              "FROM STUDENTS s "
              "LEFT JOIN USER_PROGRESS up ON s.id = up.user_id "
              "WHERE s.instructor_id = :inst_id "
              "ORDER BY s.id DESC");
...
void InstructorDashboard::showPendingRequests()
{
    ThemeManager* theme = ThemeManager::instance();
    bool isDark = theme->currentTheme() == ThemeManager::Dark;
    
    QDialog *dialog = new QDialog(this);
    dialog->setWindowTitle("Pending Registration Requests");
    ...
    QLabel *title = new QLabel("🔔 Pending Student Requests");
    title->setStyleSheet(QString("QLabel { color: %1; font-size: 20px; font-weight: bold; }").arg(theme->primaryTextColor()));
    layout->addWidget(title);
    ...
    contentLay->setContentsMargins(0, 0, 10, 0);
    
    int instructorId = qApp->property("currentUserId").toInt();
    
    auto loadRequests = [=]() {
        QLayoutItem *child;
        while ((child = contentLay->takeAt(0)) != nullptr) {
            if (child->widget()) delete child->widget();
            delete child;
        }
        
        QSqlQuery q;
        q.prepare("SELECT id, first_name || ' ' || last_name, email, phone, TO_CHAR(date_of_birth, 'DD/MM/YYYY'), TO_CHAR(enrollment_date, 'DD/MM/YYYY HH24:MI') "
                  "FROM STUDENTS WHERE instructor_id = :id AND student_status = 'PENDING' ORDER BY enrollment_date DESC");
        q.bindValue(":id", instructorId);
        ...
        if (q.exec()) {
            while (q.next()) {
                ...
                QString dob = q.value(4).toString();
                QString reqDate = q.value(5).toString();
                
                QFrame *card = new QFrame();
                card->setStyleSheet(QString("QFrame { background-color: %1; border-radius: 10px; border: 1px solid %2; }").arg(theme->cardColor(), theme->borderColor()));
                QHBoxLayout *cardLay = new QHBoxLayout(card);
                cardLay->setContentsMargins(15, 15, 15, 15);
                
                QVBoxLayout *infoLay = new QVBoxLayout();
                QLabel *nameLbl = new QLabel("<b>" + name + "</b>");
                nameLbl->setStyleSheet(QString("QLabel { color: %1; font-size: 15px; border: none; }").arg(theme->primaryTextColor()));
                QLabel *detailsLbl = new QLabel(QString("📧 %1   📞 %2   🎂 %3").arg(email, phone, dob));
                detailsLbl->setStyleSheet(QString("QLabel { color: %1; font-size: 12px; border: none; }").arg(theme->secondaryTextColor()));
                QLabel *dateLbl = new QLabel("Requested: " + reqDate);
                dateLbl->setStyleSheet(QString("QLabel { color: %1; font-size: 11px; border: none; }").arg(theme->secondaryTextColor()));
                
                infoLay->addWidget(nameLbl);
                infoLay->addWidget(detailsLbl);
                infoLay->addWidget(dateLbl);
                cardLay->addLayout(infoLay);
                cardLay->addStretch();
                
                QPushButton *acceptBtn = new QPushButton("✅ Accept");
                acceptBtn->setStyleSheet("QPushButton { background-color: #10B981; color: white; border-radius: 6px; padding: 8px 15px; font-weight: bold; border: none; } QPushButton:hover { background-color: #059669; }");
                acceptBtn->setCursor(Qt::PointingHandCursor);
                
                QPushButton *rejectBtn = new QPushButton("❌ Reject");
                rejectBtn->setStyleSheet("QPushButton { background-color: #EF4444; color: white; border-radius: 6px; padding: 8px 15px; font-weight: bold; border: none; } QPushButton:hover { background-color: #DC2626; }");
                rejectBtn->setCursor(Qt::PointingHandCursor);
                
                connect(acceptBtn, &QPushButton::clicked, [=]() {
                    QSqlQuery uq;
                    uq.prepare("UPDATE STUDENTS SET student_status = 'ACTIVE' WHERE id = :sid");
                    uq.bindValue(":sid", studentId);
                    if (uq.exec()) {
                        card->hide();
                        card->deleteLater();
                        // Optional: if the content layout becomes empty after this, we could show emptyLbl, but it's simpler to just let them see it empty.
                    }
                });
                
                connect(rejectBtn, &QPushButton::clicked, [=]() {
                    QSqlQuery uq;
                    uq.prepare("UPDATE STUDENTS SET student_status = 'REJECTED' WHERE id = :sid");
                    uq.bindValue(":sid", studentId);
                    if (uq.exec()) {
                        card->hide();
                        card->deleteLater();
                ...
                cardLay->addWidget(acceptBtn);
                cardLay->addWidget(rejectBtn);
                contentLay->addWidget(card);
            }
        }
        
        if (!hasData) {
            QLabel *emptyLbl = new QLabel("No pending requests at the moment.");
            emptyLbl->setStyleSheet(QString("QLabel { color: %1; font-size: 14px; font-style: italic; }").arg(theme->secondaryTextColor()));
            emptyLbl->setAlignment(Qt::AlignCenter);
            contentLay->addWidget(emptyLbl);
        }
    ...
    QPushButton *closeBtn = new QPushButton("Close");
    closeBtn->setStyleSheet(QString("QPushButton { background-color: %1; color: %2; border-radius: 8px; padding: 10px; font-weight: bold; border: 1px solid %3; }")
                            .arg(isDark ? "#374151" : "#F3F4F6", theme->primaryTextColor(), theme->borderColor()));
    closeBtn->setCursor(Qt::PointingHandCursor);
    connect(closeBtn, &QPushButton::clicked, dialog, &QDialog::accept);
